...
grid_range_check <- function(study, event_name){
  ...
  range_check <- study_filtered %>% mutate(in_range = case_when(container_type == "Plastic box (5x10)" & grid_position > 50 ~ "Out",
                                                                ...
                                                                container_type %in% c("Plastic Box (9x9)", "Cardboard box (9x9)"), grid_position > 81 ~ "Out",
...
container_contradictions <- function(study, event_name){
  ...
  #Group by container id, type, freezer, and shelf
  ...
  #Check if box appeared multiple times in the grouping - would mean multiple types or freezer/shelf
  box_multiple <- box_breakdown %>% group_by(container_matthay_lab_id) %>%
    tally() %>%
    filter(n > 1)

  #Filter for boxes that have discrepencies
